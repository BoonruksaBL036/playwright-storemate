...
test('(TC6002) ไม่กรอกคำค้นหาสินค้าเเต่เลือกเงื่อนไขกรอง (เลือกหมวดหมู่ ช่วงราคา)', async ({ page }) => {
  await page.goto('https://storemate-final.vercel.app/search');
  await page.waitForLoadState('domcontentloaded');
  await page.locator('[data-test="input-search"]').waitFor({ state: 'visible' });
