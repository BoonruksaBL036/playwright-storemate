...
test('(TC9007) กดลบออกทั้งหมดโดยไม่เลือกสินค้า  ', async ({ page }) => {
  ...
  const itemsBefore = await page.locator('[data-test="cart-item"]').count();
  await page.getByRole('button', { name: 'ลบออกทั้งหมด' }).click();

  await expect(page.getByText('กรุณาเลือกสินค้าก่อน')).toBeVisible();

  const itemsAfter = await page.locator('[data-test="cart-item"]').count();
  await expect(itemsAfter).toBe(itemsBefore);
